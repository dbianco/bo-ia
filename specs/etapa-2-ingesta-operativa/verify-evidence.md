...
5. **`POST /v1/fuentes/{clave}/ejecutar` es sincrónico y sin timeout propio**: bloquea la request hasta que `ejecutar_conector` termina. En el intento de verificación manual descripto arriba, la ejecución quedó en `en_curso` sin resolverse dentro de esta sesión (la máquina de desarrollo, bajo la contención de memoria de otros contenedores no relacionados, no permitió confirmar si el `join(timeout=900)` del conector llegó a dispararse). Un proxy o balanceador real delante de la API cortaría esa request mucho antes, dejando la fila en `en_curso` sin que nada la reconcilie. No es parte de los FR/SC de esta etapa, pero es un gap operativo real para etapas futuras: el endpoint manual debería ser asincrónico (devolver de inmediato y permitir consultar el estado después) en vez de bloquear.
